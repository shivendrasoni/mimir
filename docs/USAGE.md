...
TypeSafe is one cohesive configuration. When `TYPESAFE_API_KEY` is present it is active automatically unless `--typesafe off` is supplied; explicit `--typesafe on` and `--typesafe off` win. It sends bounded skill/tool selection state during a turn and, after a completed task, a separate bounded local transcript projection to Jev. Parameter schemas, thinking, images, binary data, tool arguments, and credentials are not sent.
...
`MIMIR_TYPESAFE=on|off` is the environment equivalent. Model, timeout, and feature-specific policy—including the nested `TypeSafeLearningConfig`—live inside one `TypeSafeConfig`; they are intentionally not separate CLI flags.
